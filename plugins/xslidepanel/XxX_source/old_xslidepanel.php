 ...
 *
 * @author		devXive <support@devxive.com>
 * @copyright		Copyright (C) 1997 - 2013 devXive - research and development. All rights reserved.
 * @license		GNU General Public License version 2 or later; see LICENSE.txt
 * @link		http://devxive.com
 */
 ...
 * @subpackage  System.Xafbootstrap
 * @since       2.5
 *
 */
class PlgSystemXafbootstrap extends JPlugin
{
	...
	function onAfterInitialise()
	{
		...
		{
			define('BOOTSTRAP_VERSION', '2.2.1');
		}
	...
	function onAfterRender()
	{
		...
		$loadBootstrapCss		= $this->_params->get('loadBootstrapCss', 0);
		$loadBootstrapCssOverride	= $this->_params->get('loadBootstrapCssOverride', 0);
			...
			|| ($app->isAdmin() && ($loadFrontBack == 'backend' || $loadFrontBack == 'both')) )
		{
			...
			if ($loadBootstrapCss)
			{
				...
				// Bootstrap responsive CSS - loaded in header
				$bootstrapResponsiveCss = $this->_urlCss . '/bootstrap-responsive.min.css';
				$this->_addCssCall($bootstrapResponsiveCss, $injectPosition);
			}
			// Load Bootstrap CSS Overrides?
			if ($loadBootstrapCssOverride)
			{
				// Bootstrap overrides CSS - loaded in header
				$bootstrapCssOverride = $this->_urlCss . '/bootstrap-overrides.css';
				$this->_addCssCall($bootstrapCssOverride, $injectPosition);
			}
			// Load Bootstrap JS?
			if ($loadBootstrap)
			{
	...
	private function _initFolders()
	{
		...
		$this->_urlJs           = JURI::root(true) . "/media/xaf/js";
		$this->_urlCss          = JURI::root(true) . "/media/xaf/css";
